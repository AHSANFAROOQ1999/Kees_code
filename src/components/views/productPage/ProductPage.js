...
import { Tab, Input, Button, Loader, Icon } from 'semantic-ui-react'
  ...
  fetchProduct = () => {
    Axios.get(
      ...
      .then((response) => {
        ...
        // if soldout
        if (this.state.product.sold_out) {
          this.setState({
  ...
  render() {
    ...
    return (
      <div className='product-page-wrapper'>
        <Helmet>
          <title>{this.state.product.seo_title ? (this.state.product.seo_title + " | KEES") : (this.state.product.title ? this.state.product.title + " | KEES" : "KEES | Best Online Shopping in Qatars")}</title>
          <meta name="description" content={this.state.product.seo_description} />
